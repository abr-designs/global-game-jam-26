    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            half4 frag(Varyings IN) : SV_TARGET
            {
                ...
                float linearDepth01 = Linear01DepthFromNear(rawDepth, _ZBufferParams);

                // Scale the thickness based on depth
                float distanceScale = 1.0 / max(linearDepth, 0.001);
                float scaledThickness = distanceScale * 5.0;
                scaledThickness = clamp(scaledThickness, 2.0, 6.0);

                // Generate 4 diagonally placed samples.
                const float half_width_f = scaledThickness * 0.5; //floor(scaledThickness * 0.5);
                const float half_width_c = scaledThickness * 0.5; //ceil(scaledThickness * 0.5);
                ...
                // float ndotv = abs(dot(centerNormal, viewDir));
                // float angleAdjustedThreshold = normal_threshold / max(ndotv, 0.1);
                ...
                float edge = max(edge_depth, edge_normal);//angleAdjustedThreshold);
